...
export class LiveBedrockAdapter implements ConversationBedrockAdapter {
  ...
  async generateResponse(systemPrompt: string, userContext: string): Promise<string> {
    ...
    try {
      ...
      span.setAttribute('bedrock.prompt_tokens', responseBody.usage?.input_tokens ?? 0)
      span.setAttribute('bedrock.completion_tokens', responseBody.usage?.output_tokens ?? 0)
