...
describe('ExplainRiskHandler', () => {
  ...
  it('delegates to Bedrock with incident severity and token usage', async () => {
    ...
    mockBedrock.generateResponse.mockResolvedValueOnce({
      text: 'The friction is high causing bearing wear.',
      usage: { input_tokens: 250, output_tokens: 35, total_tokens: 285 },
      model_id: 'test-model',
    })
    ...
    expect(res.meta?.input_tokens).toBe(250)
    expect(res.meta?.total_tokens).toBe(285)
  })
  ...
  it('uses critical severity from risk_state when no incident', async () => {
    ...
    mockIncident.findActiveIncident.mockResolvedValueOnce(null)
    mockBedrock.generateResponse.mockResolvedValueOnce({
      text: 'Pressure is dangerously high.',
