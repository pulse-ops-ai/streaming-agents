...
describe('thermal_runaway scenario', () => {
  it('temperature stable at ~38°C through tick 89, ramps to ~55°C by tick 180', () => {
    const events = generateEvents('thermal_runaway', 'tr-test', 180)

    // Phase 1 (tick 30): healthy baseline around 38°C
    const earlyTemp = events[30].board_temperature_c as number
    expect(earlyTemp).toBeGreaterThan(34)
    expect(earlyTemp).toBeLessThan(42)

    // Tick 89: still in baseline phase
    const preOnsetTemp = events[89].board_temperature_c as number
    expect(preOnsetTemp).toBeGreaterThan(34)
    expect(preOnsetTemp).toBeLessThan(42)

    // Tick 179: should be near 55°C (38 + 17 = 55, between warn=50 and critical=60)
    const lastTemp = events[179].board_temperature_c as number
    expect(lastTemp).toBeGreaterThan(50)
    expect(lastTemp).toBeLessThan(60)
  })
})
