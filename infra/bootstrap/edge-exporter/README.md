...
- Lex bot `YOUR_BOT_ID` must exist (created by main infrastructure)
...
- **Resource**:
  - `arn:aws:lex:REGION:ACCOUNT_ID:bot-alias/YOUR_BOT_ID/*` (all aliases)
  - `arn:aws:lex:REGION:ACCOUNT_ID:bot/YOUR_BOT_ID` (bot itself)
...
- Read/interact access to a specific Lex bot (YOUR_BOT_ID only)
...
The policy is scoped to the specific streaming-agents Lex bot (ID: `YOUR_BOT_ID`):
...
1. Verify the bot is deployed: `aws lexv2-models describe-bot --bot-id YOUR_BOT_ID`
2. Check the bot alias exists: `aws lexv2-models list-bot-aliases --bot-id YOUR_BOT_ID`
...
5. Test Lex: `aws lexv2-runtime recognize-text --bot-id YOUR_BOT_ID --bot-alias-id TSTALIASID --locale-id en_US --session-id test --text hello`
